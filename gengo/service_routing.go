...
func generateOperationParametersParsing(w *gen.Writer, operation *spec.NamedOperation, namedParams []spec.NamedParam, paramsParserName string, paramName string) {
	if namedParams != nil && len(namedParams) > 0 {
		...
		w.Line(`  log.WithFields(%s).Warnf("Can't parse %s: %%s", %s.Errors)`, paramsParserName, paramsParserName)
...
func generateOperationMethod(w *gen.Writer, operation *spec.NamedOperation) {
	...
	if operation.Body != nil {
		...
		w.Line(`if err != nil {`)
		w.Line(`  log.WithFields(%s).Warnf("Decoding body JSON failed: %%s", err.Error())`)
		w.Line(`  res.WriteHeader(400)`)
		w.Line(`  log.WithFields(%s).WithField("status", 400).Info("Completed request")`, logFieldsName(operation))
		w.Line(`  return`)
		w.Line(`}`)
	...
	w.Line(`  if err != nil {`)
	w.Line(`    log.WithFields(%s).Errorf("Error returned from service implementation: %%s", err.Error())`)
	w.Line(`  } else {`)
	w.Line(`    log.WithFields(%s).Errorf("No result returned from service implementation")`)
	w.Line(`  }`)
	...
	w.Line(`log.WithFields(%s).Error("Result from service implementation does not have anything in it")`)
	w.Line(`res.WriteHeader(500)`)
	w.Line(`log.WithFields(%s).WithField("status", 500).Info("Completed request")`, logFieldsName(operation))
	w.Line(`return`)
}
