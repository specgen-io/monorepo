...
func generateController(version *spec.Version, api *spec.Api, apiPackage Module, jsonPackage Module, modelsVersionPackage Module, serviceVersionPackage Module) []gen.TextFile {
	...
	w.Line(`  private static final Logger LOG = LogManager.getLogger(%s.class);`, className)
	w.EmptyLine()
	w.Line(`  @Autowired`)
...
func generateMethod(w *gen.Writer, version *spec.Version, api *spec.Api, operation spec.NamedOperation) {
	...
	if operation.Body != nil {
		...
		w.Line(`    LOG.error("Completed request with status code: {}", HttpStatus.BAD_REQUEST);`)
	...
	if len(operation.Responses) == 1 {
		for _, resp := range operation.Responses {
			if resp.Type.Definition.IsEmpty() {
				...
				w.EmptyLine()
				w.Line(`  LOG.info("Completed request with status code: {}", HttpStatus.%s);`, resp.Name.UpperCase())
	...
	if len(operation.Responses) > 1 {
		...
		w.EmptyLine()
		w.Line(`  LOG.error("Completed request with status code: {}", HttpStatus.INTERNAL_SERVER_ERROR);`)
