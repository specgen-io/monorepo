...
func generateApis(spec *spec.Spec) *yamlx.YamlMap {
	paths := yamlx.NewYamlMap()
	groups := OperationsByUrl(spec)
	for _, group := range groups {
		path := yamlx.NewYamlMap()
